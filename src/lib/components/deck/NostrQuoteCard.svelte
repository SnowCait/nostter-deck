<script lang="ts">
	...
	let { href, eventId, relayHints, textClass, avatarShape, getProfile }: Props = $props();

	const state = $derived(getNostrQuoteState(eventId));
	const quotedPost = $derived(
		state?.status === 'loaded'
			? eventToPost(state.event, getProfile(state.event.pubkey))
			: undefined
	);
