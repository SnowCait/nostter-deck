<script lang="ts">
	import { Braces, Ellipsis, Share } from '@lucide/svelte';
	...
	type Props = {
		...
		canShare?: boolean;
		onShare?: () => Promise<SharePostResult> | SharePostResult;
		muteLabel?: string;
		onMute?: () => void;
		class?: string;
	};
	...
	let shareStatus = $state<'idle' | 'shared' | 'failed'>('idle');
...
{#if entries.length > 0}
	...
	<p class="sr-only" aria-live="polite">
		{shareStatus === 'shared' ? m.shared() : shareStatus === 'failed' ? m.share_failed() : ''}
	</p>
{/if}
